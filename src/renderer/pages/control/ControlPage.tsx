...
const emptyDeck: MixDeck = { type: null, assetId: null, opacity: 0, enabled: false, playing: false };
...
const ControlPage = () => {
  ...
  useEffect(() => {
    const unsubscribes = deckKeys.map((deckKey) => {
      ...
      return addEventListener(deckId, (state, details) => {
        ...
        const normalizeCacheBustedSrc = (value: string | null) => {
          if (!value) {
            return null;
          }
          try {
        ...
        const candidateSrcRaw =
          typeof details?.src === 'string' && details.src.trim().length > 0
            ? details.src.trim()
            : typeof managerState.src === 'string'
              ? managerState.src
              : null;
        const candidateSrc = normalizeCacheBustedSrc(candidateSrcRaw);
        const remoteSrc = normalizeCacheBustedSrc(remoteDeckMediaStates[deckKey]?.src ?? null);
        if (candidateSrc && !remoteSrc && candidateSrc !== reportedDeckSrcRef.current[deckKey]) {
    ...
    const loadDecks = async () => {
      for (const deckKey of deckKeys) {
        ...
        try {
          ...
          const normalizedDeckSrc = normalizeMediaSrc(deckManagerState.src);
          const normalizedMasterSrc = normalizeMediaSrc(masterManagerState.src);

          if (normalizedTarget) {
            if (normalizedDeckSrc !== normalizedTarget) {
              await loadSource(deckId, targetSrc);
              if (cancelled) return;
            }
            if (normalizedMasterSrc !== normalizedTarget) {
  ...
  }, [getState, loadSource, normalizeMediaSrc, remoteDeckMediaStates, resolveDeckAssetSrc]);
  ...
  const renderDeckPreviewContent = (deckKey: DeckKey) => {
    ...
    if (deck.type === 'video' && deck.assetId) {
      ...
      if (video && 'url' in video) {
        ...
        return (
          <div key={videoKey} className="deck-preview-video-wrapper">
            <div
            ...
            {effectiveOpacity <= 0.001 && (
              <div className="deck-preview-placeholder deck-muted">Deck muted</div>
            )}
          </div>
        );
      }
