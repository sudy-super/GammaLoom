...
import {
  createDefaultDeckTimelineState,
  type DeckMediaStateIntent,
  type RTCSignalMessage,
  type StartVisualizationPayload,
} from '../types/realtime';
...
const ViewerPage = () => {
  ...
  const normalizeMediaSrc = useCallback((value: string | null | undefined) => {
    if (!value) {
      return null;
    }
    try {
  ...
  const resolveEffectiveMediaState = useCallback(
    (deckKey: DeckKey) => {
      const deckConfig = mixState?.decks?.[deckKey];
      const timeline = deckMediaStates?.[deckKey];
      if (!deckConfig && !timeline) {
        return undefined;
      }

      const base = timeline ? { ...timeline } : { ...createDefaultDeckTimelineState() };
      const fallbackSrc = deckConfig?.sourceUrl ?? resolveDeckAssetSrc(deckKey);
      if (!base.src && fallbackSrc) {
      ...
      const deckPlaying =
        typeof deckConfig?.playing === 'boolean' ? deckConfig.playing : undefined;
      const finalPlaying =
        typeof deckPlaying === 'boolean'
          ? deckPlaying
          : typeof timelinePlaying === 'boolean'
            ? timelinePlaying
            : Boolean(deckConfig?.enabled && deckConfig?.type);

      const timelineRate =
        typeof base.playRate === 'number' && Number.isFinite(base.playRate) && base.playRate > 0
          ? base.playRate
          : undefined;
      const finalRate = timelineRate ?? deckConfig?.speed ?? 1;
    ...
    [deckMediaStates, mixState?.decks, resolveDeckAssetSrc],
  );
  ...
  useEffect(() => {
    ...
    MIX_DECK_KEYS.forEach((deckKey) => {
      ...
      const normalizedTarget = normalizeMediaSrc(targetSrc);
      const normalizedManager = normalizeMediaSrc(managerState.src);

      if (normalizedTarget) {
        if (normalizedManager !== normalizedTarget) {
      ...
      if (duration && Number.isFinite(duration) && duration > 0) {
        const elapsed =
          state.isPlaying && Number.isFinite(state.updatedAt)
            ? Math.max(0, nowSeconds - state.updatedAt)
            : 0;
      ...
      const shouldPlay = state.isPlaying && Boolean(normalizedTarget);
      if (shouldPlay) {
